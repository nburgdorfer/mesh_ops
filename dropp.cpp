/* Eric Joyce, Stevens Institute of Technology, 2019; ejoyce@stevens.edu
   ...
   19dec19:  Receives a mesh and returns a reduced mesh that has dropped the N% largest triangles.
...
/*
#define __DROPAREA_DEBUG 1
*/
/*
#define __DROPAREA_LOAD_DEBUG 1
*/
...
/*
g++ -Wall dropp.cpp -lm -o dropp
./dropp delivery_area.mincut.KB.ply 0.01
*/
...
int main(int argc, char* argv[])
  {
    ...
    double drop = 0.1;                                              //  Portion of the triangles to drop
    unsigned int* fileInfo;                                         //  Array of data about the given PLY file
    unsigned char fileInfoLen;                                      //  Length of that array

    double* areas;                                                  //  Array of Face areas sorted from SMALLEST to LARGEST in area
    ...
    if(argc > 2)                                                    //  This argument is optional:
      drop = (double)atof(argv[2]);                                 //  If given a 'drop', use that
    if(drop > 1.0 || drop < 0.0)
      {
        usage();
        return 0;
      }
    ...
    writePLY("dropp.ply", &V, F, fileInfo, indices, drop);          //  Create the new file
...
void writePLY(const char* filename, Vertex** V, Face* F, unsigned int* info, unsigned int* faceIndices, double drop)
  {
    ofstream fh;                                                    //  Output file handle
    unsigned int cutoff;                                            //  Length (not the index) of the diminished Face array
    ...
    cutoff = (unsigned int)round((1.0 - drop) * (double)info[1]);   //  Compute the index of indices of Faces to keep:
                                                                    //  indices of Faces to be kept are in faceIndices[0:cutoff)
...
void quicksort(double** areas, unsigned int** indices, unsigned int lo, unsigned int hi)
  {
    ...
    if(lo < hi)
      {
        p = partition(false, areas, indices, lo, hi);               //  Sort ascending by area
...
void usage(void)
  {
    cout << "Usage:  ./dropa ply-filename <drop-portion>" << endl;
    cout << " e.g.:  ./dropa terrains.ply" << endl;
    cout << " e.g.:  ./dropa terrains.ply 0.1" << endl;
    cout << "Writes a new file \"dropa.ply\" equal to the original, minus the larger triangles and any now-unnecessary vertices." << endl;
    return;
  }
